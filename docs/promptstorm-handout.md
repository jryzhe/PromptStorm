...
PromptStorm 是一個可以在終端機中同時操作兩個大語言模型的 CLI 工具。使用者輸入兩個角色或立場，再輸入一個主題，系統就會讓 Player A 與 Player B 輪流回應，形成辯論、討論或角色對話。過程中，使用者可以隨時選擇目前比較支持哪一方、補充自己的意見、繼續更多回合，或輸出最後整理。

這個工具的核心想法是把平常手動做的「把 A LLM 的答案丟給 B LLM，再把 B 的答案丟回 A」自動化，讓多模型互相激盪的流程變成一個可以持續互動的終端機體驗。
...
- `models.py`：定義資料結構。
- `modes.py`：定義 debate、discussion、dialogue 三種模式。
...
### 3. 接上 Vercel AI Gateway

模型呼叫透過 Vercel AI Gateway 完成。`provider.py` 使用 OpenAI Python SDK 的相容介面，把 `base_url` 指到：

```text
https://ai-gateway.vercel.sh/v1
```

所以程式可以用類似 OpenAI chat completions 的方式串接不同模型。
...
這個資料結構描述一種模式需要的所有 prompt 和 UI 文案。例如：

- `debate` 強調攻防和反駁。
- `discussion` 強調合作分析和整合。
...
適合共同分析問題。兩方不是互相攻擊，而是從不同角度補足盲點，最後輸出 synthesis。
...
`VercelGatewayProvider` 是目前實際使用的 provider。它做了幾件事：

1. 延遲建立 OpenAI client。
2. 呼叫 Vercel AI Gateway。
3. 使用 streaming 逐段收集模型輸出。
...
使用 streaming 的好處是，CLI 可以即時印出模型文字，而不是等完整回應產生完才顯示。
...
- `provider`：實際呼叫模型的物件。
- `rounds`：初始要跑幾回合。
- `mode`：使用 debate、discussion 或 dialogue。
...
以下是 PromptStorm 一次 `debate` session 的流程：

```text
使用者輸入 promptstorm debate
        |
        v
cli.main() 解析指令
        |
