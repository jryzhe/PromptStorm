...
class CliEntrypointTests(unittest.TestCase):
    ...
    def test_dialogue_run_session_user_flow_writes_audit_after_control_choices(self):
        ...
        self.assertIn("模擬收尾：dialogue / TIE / 13 turns", stdout)
        self.assertNotIn("Your choice >", stdout)
        self.assertNotIn("Your input >", stdout)
        self.assertEqual(len(turn_rows), 13)
        self.assertEqual(turn_rows[6]["speaker"], "USER")
        self.assertEqual(turn_rows[6]["response_text"], "讓 B 不要急著給建議，先問 A 真正害怕的是什麼。")
        self.assertEqual(turn_rows[7]["speaker"], "B")
        self.assertEqual(turn_rows[8]["speaker"], "A")
